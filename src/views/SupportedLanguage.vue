...
<hr/>
  <!-- Table -->
      ...
      <tr v-for="(lang, index) in languages"
        ...
        <td>{{lang.code}}</td>
        <td>
          <span class="me-2" @click="showEditModal(lang)" role="button"
    ...
    <!-- add languages -->
<base-modal :modalState="isAddModalVisible" title="Language" @close="closeAddModal" btnLabel="Save" 
  :isLoading="isLoading" @submit="addNewLanguage">
     <form @submit.prevent>
  ...
  <base-modal :modalState="isDeleteModalVisible" btnLabel="Delete" :isLoading="isLoading"
   title="Delete Image" @close="closeDeleteModal" @submit="deleteLanguage">
...
export default {
    ...
    async updateLanguage(){
      ...
      if (!this.v$.$error) {
        this.isLoading = true;
        try {
          const response = await apiClient.put(`/api/languages/${this.language.id}`, this.language);
