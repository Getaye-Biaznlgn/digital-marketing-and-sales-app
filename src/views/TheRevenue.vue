      ...
      <div class="row gx-5 gy-3">
        <div class="col">
          <div class="bg-blue-black py-2 px-4">
            <div class="my-2 fw-bold">Recent Sales</div>
            <!-- Table -->
            <table class="mt-2">
              <tr>
                <th>No</th>
...
const fetchOrders = async function () {
  try {
    ...
    const response = await apiClient.get(`/api/sales?per_page=5`);
...
const fetchCustomerStatistics = async function () {
  try {
    ...
    const response = await apiClient.get(
      `/api/revenue_bargraph?time=${selectedTimeForBar.value}`
    );
    if (response.status === 200) {
      let graphLables = [];
      let graphValues = [];
...
const fetchSoldProductStatistics = async function () {
  try {
    store.commit("setIsLoading", true);
    const response = await apiClient.get(
      `/api/revenue_piechart?time=${selectedTimeForPie.value}`
    );
    if (response.status === 200) {
      let graphLables = [];
      let graphValues = [];
