<template>
<button @click="showSendModal" class="btn border ms-auto d-block">Send Product</button>
<!-- Table -->
      <table class="mt-2">
        <tr>
          <th>No</th>
  ...
  <input
    ...
    />
     ...
     <li v-for="(product, index) in filteredProduct" :key="product.id" class="my-2 p-2 border-bottom shadow-sm d-flex justify-content-between">
      ...
      <div class="">{{index+1+'.' +' '+product.model}}</div>
      <input min="1" type="text" size="4" class="p-0" @blur="setValue(product.id,$event)"  name="qty" :id="'qty'+key">
...
export default {
  ...
  computed:{
   ...
   filteredProduct(){
      ...
      //        product.name.toString().toLowerCase().contains(this.searchByName.toString().toLowerCase()));
      }
  ...
  methods:{
    ...
    setValue(productId, event){
      let index= this.selectedProductQty.findIndex((product)=>product.product_id.toString()==productId.toString())
      if(index==-1){
        if(event.target.value!=''){
        this.selectedProductQty.push({product_id: productId, qty: event.target.value});
      }else{
   ...
   showSendModal(){
    ...
    async sendProductsToAgent(){
        this.isLoading = true;
        try {
          const response = await apiClient.post("/api/product_distribution_data", {
           shop_id: this.shopId,
